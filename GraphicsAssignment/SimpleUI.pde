public class SimpleUI {
  ...
  // menu creation
  public Menu addMenu(String label, int x, int y, int w, int h, String[] menuItems) {
    if (widgetNameAlreadyExists(label, "Menu")) return null;
    Menu m = new Menu(UIManagerName, label, x, y, w, h, menuItems, this);
...
class Widget {
  ...
  color SimpleUIBackgroundRectColor = color(100, 100, 100); // slightly purpley background rect Color for alternative UI's
  color SimpleUIWidgetFillColor = color(100, 100, 100);// darker grey for butttons
  color SimpleUIWidgetRolloverColor = color(115, 115, 115);// slightly lighter rollover Color
  color SimpleUITextColor = color(255, 255, 255);
  color SimpleUIBorderColor = color(0, 0, 0);
...
class SimpleLabel extends Widget {
  ...
  public SimpleLabel(String uiname, int x, int y, String uilable, String txt) {
    super(uiname, uilable, x, y, 110, 20);
...
class Menu extends Widget {
  ...
  public Menu(String uiname, String uilabel, int x, int y, int w, int h, String[] menuItems, SimpleUI manager) {

    super(uiname, uilabel, x, y, w, h);
    parentManager = manager;
  ...
  void drawItems() {
    ...
    rect(locX, thisY, 100, (widgetHeight*numItems));
    ...
    for (String s : itemList) {
      ...
      text(s, locX+textPad, thisY, 100, widgetHeight);
      thisY += widgetHeight;
    }
    popStyle();
  }
  ...
  void hiliteItem(int y) {
    ...
    fill(115, 115, 115);
    rect(locX, topOfItems + itemNum*widgetHeight, 100, widgetHeight);
    popStyle();
  }
...
class Slider extends Widget {
  ...
  float getSliderValueAtMousePos(int pos) {
    float val = map(pos, bounds.left, bounds.right, 0, 2);
    return val;
  }
  ...
  void setSliderValue(float val) {
    currentValue =  constrain(val, 0, 2);
  }
  ...
  public void drawMe() {
    ...
    rect(bounds.left, bounds.top, bounds.getWidth() + 8, bounds.getHeight());
    fill(SimpleUITextColor);
    textSize(textSize);
    text(this.UILabel, bounds.left+textPad, bounds.top+26);
    int sliderHandleLocX = (int) map(currentValue, 0, 2, bounds.left, bounds.right);
    sliderHandleLocX = (int)constrain(sliderHandleLocX, bounds.left+10, bounds.right );
    stroke(127);
    float lineHeight = bounds.top+ (bounds.getHeight()/2.0) - 5;
    line(bounds.left+5, lineHeight, bounds.left+bounds.getWidth(), lineHeight);
    stroke(0);
